...
class Qordoba_Module_Polylang extends Qordoba_Module {
	...
	public function save_post_translation( $post_id, $language_code, $translation ) {
		...
		if ( $tr_post_id = PLL()->model->post->get( $source_id, $language_code ) ) {
			...
			if ( isset( $translation['elementor'] ) ) {
				...
				} elseif ( $this->is_pagebuilder_plugin_exists() ) {
					$this->save_pagebuilder_data( $tr_post_id, $translation['elementor'] );
				}
			}
			...
			if ( $source->post_parent && $tr_parent = PLL()->model->post->get_translation( $source->post_parent,
					$language_code ) ) {
			...
			if ( 'attachment' == $source->post_type ) {
				...
				add_post_meta( $tr_post_id, '_wp_attached_file',
					get_post_meta( $source_id, '_wp_attached_file', true ) );
				add_post_meta( $tr_post_id, '_wp_attachment_metadata',
					get_post_meta( $source_id, '_wp_attachment_metadata', true ) );
			} else {
			...
			if ( $tr_post_id ) {
				...
				PLL()->sync->copy_taxonomies( $source_id, $tr_post_id, $language_code );
				PLL()->sync->copy_post_metas( $source_id, $tr_post_id, $language_code, false );
	 ...
	 * @param array $data
	 */
	public function save_pagebuilder_data( $post_id, $data = [] ) {
		$pagebuilderPostMeta = get_post_meta( $post_id, '_fl_builder_data' );
		$source_id           = $this->get_source_post_id( $post_id );
		if ( ! $pagebuilderPostMeta ) {
			$pagebuilderPostMeta = get_post_meta( $source_id, '_fl_builder_data' );
		}
		if ( is_array( $pagebuilderPostMeta ) && isset( $pagebuilderPostMeta[0] ) ) {
			foreach ( $data as $key => $value ) {
				$tmp = explode( '::', $key );
				if (isset($tmp[0], $tmp[1], $pagebuilderPostMeta[0][$tmp[0]]->settings)) {
					if ('text' === $tmp[1]) {
						$pagebuilderPostMeta[0][$tmp[0]]->settings->text = $value;
					} elseif ('after_number_text' === $tmp[1]) {
						$pagebuilderPostMeta[0][$tmp[0]]->settings->after_number_text = $value;
					} elseif ('before_number_text' === $tmp[1]) {
						$pagebuilderPostMeta[0][$tmp[0]]->settings->before_number_text = $value;
					}  elseif ('heading' === $tmp[1]) {
						$pagebuilderPostMeta[0][$tmp[0]]->settings->heading = $value;
					} elseif ('testimonial' === $tmp[1]) {
						$pagebuilderPostMeta[0][$tmp[0]]->settings->testimonials[$tmp[2]]->testimonial = $value;
					}
				}
			}
			update_post_meta( $post_id, '_fl_builder_enabled', 1 );
			update_post_meta( $post_id, '_fl_builder_data', $pagebuilderPostMeta[0] );
		}
	}
