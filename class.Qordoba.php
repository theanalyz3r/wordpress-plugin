...
class Qordoba
{
    ...
    public function send_post($post_id, $languages = false)
    {
        ...
        if ($this->isElementorPluginExists()) {
            $meta = $this->getMetaData($post_id);
            $metaData = array();
            foreach ($meta as $item) {
                $itemData = json_decode($item, true);
                $metaData[] = $this->recursiveMetaParser($itemData);
                $flatMetaData = call_user_func_array('array_merge', $metaData);
            }
            $object = new Qordoba_Object($source_post, $this->prepareQordobaMetaData($flatMetaData));
            $object->upload();
        } else {
     ...
     * @return array
     */
    private function prepareQordobaMetaData($elements = array())
    {
        $qordobaMetaData = array();
        foreach ($elements as $id => $item) {
            if (isset($item['title'])) {
                $qordobaMetaData[$id . "_title"] = trim(strip_tags($item['title']));
            }
     ...
     * @return array
     */
    private function recursiveMetaParser($element = array(), $elementsData = array())
    {
     ...
     * @return bool
     */
    private function isElementorPluginExists()
    {
     ...
     * @return mixed
     */
    private function getMetaData($post_id, $metaKey = '_elementor_data')
    {
