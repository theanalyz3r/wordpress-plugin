...
class Qordoba {
	...
	public function translated_versions_match( $object_id, $object_type = 'post' ) {
		if ( $object_type === 'post' ) {
			...
			throw new Exception( "Wrong object type $object_type, expected either post or term" );
		}
	...
	public function send_post( $post_id, $languages = false ) {
		...
		if ( $this->is_elementor_plugin_exists() ) {
			...
			$metaCustomData = array();

			if ( is_array( $meta ) && 0 < count( $meta ) ) {
				foreach ( $meta as $item ) {
					$itemData         = is_string( $item ) ? json_decode( $item, true ) : $item;
					$metaCustomData[] = $this->recursive_meta_parser( $itemData );
					$flatMetaData     = call_user_func_array( 'array_merge', $metaCustomData );
				}
			...
			$meta           = $this->get_post_meta_data( $post_id, '_fl_builder_data' );
			$metaCustomData = array();
			if ( isset( $meta[0] ) ) {
				foreach ( $meta[0] as $item ) {
					if ( isset( $item->settings ) ) {
						if ( isset( $item->settings->after_number_text ) && ( '' !== $item->settings->after_number_text ) ) {
							$metaCustomData[ $item->node . '::after_number_text' ] = trim( $item->settings->after_number_text );
						}
						if ( isset( $item->settings->before_number_text ) && ( '' !== $item->settings->before_number_text ) ) {
							$metaCustomData[ $item->node . '::before_number_text' ] = trim( $item->settings->before_number_text );
						}
						if ( isset( $item->settings->text ) && ( '' !== $item->settings->text ) ) {
							$metaCustomData[ $item->node . '::text' ] = trim( $item->settings->text );
						}
						if ( isset( $item->settings->heading ) && ( '' !== $item->settings->heading ) ) {
							$metaCustomData[ $item->node . '::heading' ] = trim( $item->settings->heading );
						}
						if ( isset( $item->settings->testimonials ) && is_array( $item->settings->testimonials ) ) {
							foreach ( $item->settings->testimonials as $key => $text ) {
								$metaCustomData[ $item->node . '::testimonial' . '::' . $key ] = $text->testimonial;
							}
						}
					}
				}
				if ( 0 < count( $metaCustomData ) ) {
	 ...
	 * @return array
	 */
	private function recursive_meta_parser( array $element = array(), $elementsData = array() ) {
